...
public class InitMongoDBDataChangeLog {
    ...
    public void initFirstBook(BookRepository bookRepository,
        ...
        authorRepository.save(new Author(id, "Author_" + id)).block();
        genreRepository.save(new Genre(id,"Genre_" + id)).block();

        bookRepository.save(
          new Book(id, "Book_" + id,
                  authorRepository.findById(id).block(),
                  genreRepository.findById(id).block()
          )
        ).block();

        bookCommentsRepository.save(new BookComments(id,"Good book",
                bookRepository.findById(id).block())).block();
    }
