...
class JpaAuthorRepositoryTest {
    ...
    private List<JpaAuthor> dbAuthors;
    ...
    void shouldReturnCorrectAuthorById(JpaAuthor testAuthor) {
        var actualAuthor = repositoryJpa.findById(testAuthor.getId());
        var expectedAuthor = em.find(JpaAuthor.class, testAuthor.getId());
    ...
    private static List<JpaAuthor> getDbAuthors() {
        return IntStream.range(1, 4).boxed()
                .map(id -> new JpaAuthor(id, "Author_" + id))
                .toList();
    }
}