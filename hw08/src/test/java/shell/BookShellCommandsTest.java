...
class BookShellCommandsTest {
    ...
    void shouldExecFindByIdCommand() throws Exception {
        ...
        shell.run(inputProvider);
        verify(resultHandlerService).handle(argumentCaptor.capture());
        verify(bookService).findById(eq(BOOK_ID));
    }
    ...
    void shouldExecInsertCommand() throws Exception {
        ...
        when(bookService.insert(
                eq(BOOK_TITLE),
                eq(AUTHOR_ID),
                eq(GENRE_ID))).thenReturn(
                new Book(BOOK_ID, BOOK_TITLE,
                        new Author(AUTHOR_ID, ""),
                        new Genre(GENRE_ID, "")));

        shell.run(inputProvider);
        verify(resultHandlerService).handle(argumentCaptor.capture());
        verify(bookService).insert(eq(BOOK_TITLE), eq(AUTHOR_ID), eq(GENRE_ID));
    }
    ...
    void shouldExecDeleteCommand() throws Exception {
        ...
        shell.run(inputProvider);
        verify(resultHandlerService).handle(argumentCaptor.capture());
        verify(bookService).deleteById(eq(BOOK_ID));
    }
